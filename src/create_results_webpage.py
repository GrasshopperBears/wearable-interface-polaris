# This code is from CS 484 Computer Vision
...
def create_results_webpage(
    ...
    for i in range(num_categories):
        ...
        train_exmples = train_image_paths[train_labels == category]
        true_positives = test_image_paths[
            (test_labels == category) & (predicted_categories == category)
        ]
        false_positive_inds = (test_labels != category) & (
        ...
        false_negative_inds = (test_labels == category) & (
            predicted_categories != category
        )
        ...
        def write_samples(classname, samples, labels=None):
            ...
            for j in range(num_samples):
                if j < samples.size:
                    ...
                    row_sample_html += (
                        '<td class="%s"><img src="%s" width=%d height=%d>'
        ...
        row_html += "<td>%s</td>\n" % category
        row_html += (
            '<td><div>%.3f</div><div class="accuracy-bar" style="width:64px;">'
        ...
        row_html += write_samples(
            "false-positive", false_positives, false_positive_labels
        )
        row_html += write_samples(
            "false-negative", false_negatives, false_negative_labels
        )
        row_html += "</tr>\n"
    ...
    with open("others/template.html", "r") as template_file:
